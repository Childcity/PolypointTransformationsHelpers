...
def spiral_screw_an_obj(INPUT, div, rnd_d):
    ...
    for v in vs:
        ...
        # d1, d2, d3, d8 = rnd_d[0], rnd_d[1], rnd_d[2], rnd_d[3]
        # s1 = (1-x)*y*z; 
        # s2 = x*(1-y)*z;
        # s3 = x*y*(1-z);
        # s8 = (1-x)*(1-y)*(1-z);
        # new_x = x + d1*s1 + d2*s2 + d3*s3 + d8*s8
        # new_y = y + d1*s1 + d2*s2 + d3*s3 + d8*s8
        # new_z = z + d1*s1 + d2*s2 + d3*s3 + d8*s8
...
[[round(random.uniform(-0.5, 0.5), 2) for i in range(4)] for j in range(3)]
